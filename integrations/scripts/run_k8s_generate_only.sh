...
: "${INTEGRATIONS_MODE:=generate_only}"
: "${INTEGRATIONS_TIMEOUT:=30}"
export INTEGRATIONS_MODE INTEGRATIONS_TIMEOUT

: "${K8S_APPLY:=0}"                         # if 1, apply overlay before testing
: "${K8S_OVERLAY:=local-generate-only}"     # matches your deploy/k8s/overlays/*
: "${K8S_CONTEXT:=}"
...
# If base URL already provided (e.g., ingress), skip port-forward.
if [[ -n "${INTEGRATIONS_BASE_URL:-}" ]]; then
  ...
  "${SCRIPT_DIR}/wait_http.sh" --base-url "${INTEGRATIONS_BASE_URL}" --path "/healthz" --timeout 240

  log "Running pytest (k8s generate-only, direct URL)..."
  run_pytest
  exit 0
fi
...
if curl -fsS "${INTEGRATIONS_BASE_URL%/}/readyz" >/dev/null 2>&1; then
  ...
  "${SCRIPT_DIR}/wait_http.sh" --base-url "${INTEGRATIONS_BASE_URL}" --path "/readyz" --timeout 240
fi

log "Running pytest (k8s generate-only)..."
run_pytest